...
def question_3():
    ...
    SELECT
        LoanTerm,
        ROUND(100.0 * SUM(CASE WHEN ApprovalStatus = 'Approved' THEN 1 ELSE 0 END) / COUNT(*),2) AS ApprovedPercentage
    FROM loans
    GROUP BY LoanTerm
    ORDER BY LoanTerm
    """
...
def question_4():
    ...
    qry = """
    SELECT
        TRIM(CustomerClass) AS CustomerClass,
        COUNT(*) AS Count
    FROM credit
    GROUP BY TRIM(CustomerClass)
    ORDER BY
        CASE TRIM(CustomerClass)
            WHEN 'A+' THEN 1
            WHEN 'A' THEN 2
            WHEN 'B' THEN 3
            ELSE 4
